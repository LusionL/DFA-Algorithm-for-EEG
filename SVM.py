...
def loadDataSet(fileName):
  ...
  for i in range(0,len(line)):
    b = line[i].split()
    dataMat.append([float(b[0]),float(b[1])])
    labelMat.append(float(b[2]))
  ...
  #       lineArr = line
  #       dataMat.append([float(lineArr[0+i]), float(lineArr[1+i])])
  #       labelMat.append(float(lineArr[2+i]))
  # return dataMat, labelMat
...
def plotFeature(dataMat, labelMat, weights, b):
  ...
  plt.show()
def main():
  trainDataSet, trainLabel = loadDataSet(r'E:\DFA\core_code\testSet.txt')
